...
export function useOnboardingStatus(): OnboardingStatus {
	const { publicKey, connected, wallet } = usePrivyWallet();
	const { user } = useStore();
	...
	const isWalletReady = isWalletConnected && !!wallet?.address;

	// Check if X account is linked
	const isXLinked = !!user?.twitterId;

	// Check if birth details are complete
	const hasBirthDetails = !!(user?.dob && user?.birthTime && user?.birthPlace);
